...
rownames(ACTIVITY_AVERAGE) <- c("WALKING", "WALKING_UPSTAIRS", "WALKING_DOWNSTAIRS", "SITTING", "STANDING", "LAYING")
## each subject
SUBJECT_AVERAGE <- WALKING_AVERAGE
for(k in 1:30){for(i in 3:81){mean(unlist(select(filter(extracted_data, subject == k),i))) -> SUBJECT_AVERAGE[k,i-2]}}

## bind ACTIVITY_AVERAGE and SUBJECT_AVERAGE

tidy_data_set <- rbind(ACTIVITY_AVERAGE, SUBJECT_AVERAGE)
